      ...
      {itemOptions.map((item: ItemOptions, index: number) => {
        return (
          <Select.Item
            key={index}
            value={item.value}
            className="outline-none cursor-pointer hover:bg-sky-300 px-2"
          >
            <Select.ItemText>{item.labelText}</Select.ItemText>
            <Select.ItemIndicator />
            {item.separator && (
              <Select.SelectSeparator className="h-px bg-slate-300" />
            )}
          </Select.Item>
        )
      })}
...
const SelectRadix: FC<SelectRadixProps> = forwardRef<
  HTMLButtonElement,
  SelectRadixProps
>(({ placeholder, itemOptions, children, ...props }, forwardedRef) => {
  return (
    <Select.Root {...props}>
      <Select.Trigger
        ref={forwardedRef}
        className="outline-none text-md border-solid border-2 border-slate-500 px-2 hover:bg-white"
      >
