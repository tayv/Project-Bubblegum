...
const openAccordion = (container, getByRole) => {
  ...
  let accordionContent = container.querySelector(
    ":where([data-state='closed']):first-of-type"
  )
...
const openSecondAccordion = (container, getByRole) => {
  ...
  let accordionContent2 = container.querySelector(
    ":where([data-state='closed']):nth-of-type(1)"
  )
...
describe("DatePicks: Sync defaultValues of related fields with custom hook", () => {
  ...
  test("DatePick defaultValues match if datePickFieldWatch changes", async () => {
    ...
    // Wait for the input value of datePickFieldSyncInput to change
    await waitFor(() => {
      // Values should not match when datePickFieldSync changes
      expect(datePickFieldWatchInput.value).not.toBe(
        datePickFieldSyncInput.value
      )
    })
  })
})
